---
sidebar_position: 1
---

# Creating Teams

Teams let you collaborate with other Chrontrack users — sharing task categories, viewing each other's time data (with the right role), and managing dependant accounts together.

![Teams page](/img/screenshots/teams.png)

## Creating a team

1. Go to **Teams** (`/teams`).
2. Fill in the **team name** and an optional **description** in the create form.
3. Click **Create**. You are automatically the team's creator.

## Inviting members

1. Select your team from the sidebar.
2. In the **Invite** field, enter the email address of the person you want to add.
3. Choose their **role** (see [Member Roles](member-roles.md)).
4. Click **Invite**.

The invitee receives a pending invitation. They can accept or decline from the banner at the top of their Teams page.

ChronTrack also sends the invitee an **email notification**. The email includes:

- The name of the team and the person who invited them
- Any **task categories** currently shared with the team
- Any **dependant accounts** the team helps manage

If the invitee already has a ChronTrack account, the email links directly to the accept page. If they don't have an account yet, the link takes them to sign up first, then automatically accepts the invitation.

![Pending invite banner](/img/screenshots/teams-invite-banner.png)

## Editing team details

Click the **edit** icon next to the team name to change the name or description. Only the team creator can do this.

## Removing a member

In the members list, click the **remove** button next to a member's name. They lose access immediately.

## Leaving a team

If you are not the creator, click **Leave team** at the bottom of the team detail panel.

## Deleting a team

Only the creator can delete a team. Click **Delete team** and confirm. All shares associated with the team are revoked.
